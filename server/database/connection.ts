...
// ============================================================================
/**
 * This is the core DB abstraction layer that replaces Convex's built-in database functions.
 ...
 * - APPI compliance logging
 * - Encryption/decryption of PII data
 * - Error handling and retry logic
 * - Performance monitoring
 *
 * This connection layer will be used by:
 * - GraphQL resolvers (replacing Convex functions)
 ...
 * - Handle APPI audit logging of DB operations manually
...
/**
 * PostgreSQL Connection Manager with APPI compliance features
 ...
 * - Logs ALL database operations for APPI compliance to appi_audit_events
 */
class PostgreSQLManager {
    ...
    try {
      ...
      this.pool.on('connect', (client: PoolClient) => {
        ...
        if (process.env.APP_CURRENT_USER_ID) {
          client.query(`SET app.current_user_id = '${process.env.APP_CURRENT_USER_ID}'`);
        }
  ...
  /**
   * Log audit events for APPI compliance. Every database operation gets logged with:
...
class RedisManager {
   ...
   */
  async cacheResponse(cacheKey: string, data: any, ttlSeconds: number = 300): Promise<void> {
 ...
 */
export async function initializeConnections(retries: number = 3): Promise<void> {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (attempt === retries) {
        throw new Error(`Failed to establish database connections after ${retries} attempts`);
      }
